<template>
    <div class="notification">
      <div class="notification-content">
        <div class="success-wrapper" v-if="type == 'success'">

          <svg class="success-icon" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
             viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">
          <g>
            <g>
              <path d="M383.841,171.838c-7.881-8.31-21.02-8.676-29.343-0.775L221.987,296.732l-63.204-64.893
                c-8.005-8.213-21.13-8.393-29.35-0.387c-8.213,7.998-8.386,21.137-0.388,29.35l77.492,79.561
                c4.061,4.172,9.458,6.275,14.869,6.275c5.134,0,10.268-1.896,14.288-5.694l147.373-139.762
                C391.383,193.294,391.735,180.155,383.841,171.838z"/>
            </g>
          </g>
          <g>
            <g>
              <path d="M256,0C114.84,0,0,114.84,0,256s114.84,256,256,256s256-114.84,256-256S397.16,0,256,0z M256,470.487
                c-118.265,0-214.487-96.214-214.487-214.487c0-118.265,96.221-214.487,214.487-214.487c118.272,0,214.487,96.221,214.487,214.487
                C470.487,374.272,374.272,470.487,256,470.487z"/>
            </g>
          </g>
          </svg>

            <h3>Success</h3>
            <p>{{ message }}</p>
            <button @click="closeNotificationModal()">Ok</button>
        </div>
      </div>
      <div @click="closeNotificationModal()" class="notification-background">
      </div>
    </div>
</template>

<script>
  export default{
    computed:{
      type(){
        return this.$store.state.status.notification.type
      },

      message(){
        return this.$store.state.status.notification.message
      }
    },

    methods:{
      closeNotificationModal(){
        this.$store.commit('status/closeNotificationModal')
      },

      startTimer(){
          setTimeout(()=>{
             this.closeNotificationModal() 
           }, 3000);
      }
    },

    mounted(){
      this.startTimer()
    }
  }
</script>

<style lang="scss" scoped>
@import "~/assets/sass/app.scss";

h1,h2,h3,h4,h5,h5,p,li,input{
  font-family:$primary-font;
  margin: 0;
}

  .notification{
    position: fixed;
    width: 100%;
    height: 100%;
    display: flex;
    justify-content: center;
    align-items: center;
    top:0;
    left: 0;
    z-index: 9999999;



    .notification-content{
      width: 500px;
      position: relative;
      z-index: 99999999;
      background-color: $white;
      padding:2rem;
      margin-right: 1rem;
      margin-left: 1rem;
      display: flex;
      justify-content: center;
      -webkit-box-shadow: 0px 0px 22px 4px rgba(0,0,0,0.27);
      box-shadow: 0px 0px 22px 4px rgba(0,0,0,0.27);

      .success-wrapper{
          text-align: center;
          display: flex;
          flex-flow: column;
          align-items: center;

          h3{
            font-size: 1.4rem;
          }

        .success-icon{
          width:3rem;
          height: auto;
          fill: $success;
        }

        button{
          @include system-button($success, $white);
          margin-top: 1rem;
        }
      }
    }

    .notification-background{
      position: fixed;
      left: 0;
      top: 0;
      width: 100%;
      height: 100vh;
      background-color: $black;
      opacity: 0.6;
      z-index: 999;
    }
  }

</style>
